defmodule LoremImpsumMd.Hugo.Generator do
  ...
  defp posts_folder do
    Path.join(@content_folder, @posts_folder)
  end
  ...
  defp generate_articles(args, category_tokens) do
    ...
    Enum.map(1..num_articles, fn n ->
      Task.async(fn ->
        ...
        File.write(Path.join(posts_folder(), title <> ".md"), LoremImpsumMd.Hugo.ArticleTemplate.tpl(title, category_tokens, []))
      end)
    end)
